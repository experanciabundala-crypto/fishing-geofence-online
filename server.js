...
// ── MPYA: Matukio ya Tamper/SOS — kuanzia yalipoanza mpaka yalipo-CLEARED ──
// incidentsLog: historia KAMILI (yaliyokwisha na yanayoendelea) - kwa ripoti
// activeIncidents: { boatId: { tamper: incidentObj|null, need_help: incidentObj|null } }
// kufuatilia haraka tukio LINALOENDELEA sasa hivi kwa kila boti
const incidentsLog = [];
const INCIDENTS_LOG_LIMIT = 2000;
const activeIncidents = {};
let incidentSeq = 0;

function startIncident(boatId, type, lat, lon, time) {
  if (!activeIncidents[boatId]) activeIncidents[boatId] = {};
  incidentSeq++;
  const incident = {
    incidentId: incidentSeq,
    boatId,
    boatName: registeredBoats[boatId]?.name || boatId,
    type, // 'tamper' au 'need_help'
    status: 'active',
    startTime: time,
    endTime: null,
  ...
  activeIncidents[boatId][type] = incident;
  incidentsLog.unshift(incident);
  if (incidentsLog.length > INCIDENTS_LOG_LIMIT) incidentsLog.pop();
  return incident;
}

function clearIncident(boatId, type, lat, lon, time) {
  const incident = activeIncidents[boatId]?.[type];
  if (!incident) return null;
  incident.status = 'cleared';
  incident.endTime = time;
  incident.durationSeconds = Math.round((new Date(time) - new Date(incident.startTime)) / 1000);
  incident.endLat = lat; incident.endLon = lon;
  activeIncidents[boatId][type] = null;
  return incident;
}
...
// MPYA — matukio ya Tamper/SOS: yanayoendelea (active) na yaliyokwisha (cleared), na muda wake
app.get('/api/incidents', (req, res) => res.json(incidentsLog));
app.get('/api/full-history', (req, res) => res.json({
  startTime: startTime.toISOString(),
  alerts: alertsLog,
  positions: positionLog,
  incidents: incidentsLog,
